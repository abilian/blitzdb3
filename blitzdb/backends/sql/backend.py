...
class Backend(BaseBackend):
    ...
    def get_field_type(self, field, name=None):
        m = {
            ...
            EnumField: lambda field: Enum(
                *field.enums, name=name, native_enum=field.native_enum
            ),
            TextField: Text,
        ...
        def add_foreign_key_field(collection, cls, key, field, backref=None):
            ...
            if backref is None:
                ...
                params["backref"] = add_one_to_many_field(
                    ...
                    related_class,
                    backref_key,
                    OneToManyField(related=cls, unique=field.unique, key=backref_key),
                    backref=params,
                )
            ...
            if backref is None:
                ...
                RelationshipClass.__name__ = str(
                    ...
                    % (
                        ...
                        "".join(
                            [
                                "".join([kk.capitalize() for kk in k.split("_")])
                                for k in key.split(".")
                            ]
                        ),
                    )
                ...
                RelationshipClass.fields[related_pk_field_name] = ForeignKeyField(
                    related_class, backref=backref_name_right, ondelete=self._ondelete
                )
                ...
                self.init_class_schema(
                    RelationshipClass, relationship_name, table=relationship_table
                )
    ...
    def update(self, obj, set_fields=None, unset_fields=None, update_obj=True):
        ...
        with self.transaction(implicit=True):
            ...
            self._serialize_and_update_indexes(
                update_dict, collection, d, for_update=True
            )
            self._serialize_and_update_relations(
                update_dict,
                collection,
                d,
                deletes,
                inserts,
                autosave_dependent=True,
                for_update=True,
            )
        ...
        for related_field, relation_params in self._related_fields[collection].items():
            ...
            try:
                ...
                if isinstance(relation_params["field"], ManyToManyField):
                    ...
                    relationship_table = self._relationship_tables[collection][
                        related_field
                    ]
                    deletes.append(
                    ...
                    for element in value:
                        if not isinstance(element, Document):
                            raise AttributeError(
                                "ManyToMany field {} contains an invalid value!".format(
                                    related_field
                                )
                            )
                        ...
                        if element.pk is None:
                            raise AttributeError(
                                "Related document in field {} has no primary key!".format(
                                    related_field
                                )
                            )
        ...
        try:
            with self.transaction(implicit=True):
                ...
                self._serialize_and_update_relations(
                    ...
                    deletes,
                    inserts,
                    autosave_dependent=autosave_dependent,
            ...
            if isinstance(params["field"], ManyToManyField):
                ...
                if foreign_key_data:
                    ...
                    try:
                        ...
                        logger.warning(
                            "Found corrupted data in related field data for key {}".format(
                                key
                            )
                        )
                        continue
        ...
        def resolve_include(include, collection, d, path=None):
            ...
            if main_include == "*":
                if sub_includes is not None:
                    raise AttributeError(
                        "Wildcard (*) include cannot be specified together with sub-includes!"
                    )
                ...
                for key, params in self._related_fields[collection].items():
                    ...
                    if sub_includes is not None:
                        raise AttributeError(
                            "Included field '{}' is not a related object!".format(
                                main_include
                            )
                        )
    ...
    def get(self, cls_or_collection, query, raw=False, only=None, include=None):
        ...
        result = self.filter(
            cls_or_collection, query, raw=raw, only=only, include=include
        )
        try:
            if len(result) > 1:
    ...
    def filter(self, cls_or_collection, query, raw=False, only=None, include=None):
        ...
        def compile_query(collection, query, table=None, path=None):
                ...
                if (
                    ...
                    if query_type == "elemMatch":
                        ...
                        if isinstance(subquery, (ManyToManyProxy, QuerySet)):
                            ...
                            raise AttributeError(
                                "$in/$nin/$all query requires a list/tuple/QuerySet/ManyToManyProxy"
                            )
            ...
            def compile_many_to_many_query(
                ...
                return compile_one_to_many_query(
                    key,
                    value,
                    field_name,
                    related_table_alias,
                    ...
                    if key == field_name:
                        ...
                        if isinstance(value, dict):
                            ...
                            where_statements.extend(
                                prepare_special_query(field_name, params, value)
                            )
                        else:
                        ...
                        if key.startswith(field_name):
                            ...
                            if isinstance(params["field"], ManyToManyField):
                                ...
                                if key == field_name:
                                    ...
                                    if path_str in joins[related_table]:
                                        related_table_alias = joins[related_table][
                                            path_str
                                        ]
                                    else:
                                        related_table_alias = related_table.alias()
