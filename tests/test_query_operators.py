...
def test_in(backend):
    ...
    assert len(backend.filter(Actor, query)) == len(
        [david_hasselhoff, charlie_chaplin, marlon_brando, leonardo_di_caprio]
    )
...
def test_lt(backend):
    ...
    assert len(backend.filter(Actor, query)) == len(
        [marlon_brando, charlie_chaplin, leonardo_di_caprio, david_hasselhoff]
    )
    # Test with float/int
...
def test_gte(backend):
    ...
    assert len(backend.filter(Actor, query)) == len(
        [marlon_brando, charlie_chaplin, david_hasselhoff]
    )
...
def test_lte(backend):
    ...
    assert len(backend.filter(Actor, query)) == len(
        [marlon_brando, leonardo_di_caprio, david_hasselhoff]
    )
    # Test with normal conditions
    ...
    assert len(backend.filter(Actor, query)) == len(
        [marlon_brando, charlie_chaplin, david_hasselhoff, leonardo_di_caprio]
    )
...
def test_exists(backend):
    ...
    assert all(
        [
            actor in backend.filter(Actor, query)
            for actor in [marlon_brando, charlie_chaplin]
        ]
    )
    ...
    assert all(
        [
            actor in backend.filter(Actor, query)
            for actor in [leonardo_di_caprio, david_hasselhoff]
        ]
    )
...
def test_all(backend):
    ...
    marlon_brando = Actor(
        {
            'name': 'Marlon Brando',
            'favorite_food': [pizza, spaghetti, focaccia],
            'gross_income_m': 1.453,
            'appearances': 78,
            'is_funny': False,
            'birth_year': 1924,
    ...
    leonardo_di_caprio = Actor(
        {
            'name': 'Leonardo di Caprio',
            'favorite_food': [hamburger, pizza, spaghetti],
            'gross_income_m': 12.453,
            'appearances': 34,
            'is_funny': False,
            'birth_year': 1974,
    ...
    david_hasselhoff = Actor(
        {
            'name': 'David Hasselhoff',
            'favorite_food': [hamburger, weisswurst],
            'gross_income_m': 1.0,
            'appearances': 173,
            'is_funny': True,
            'birth_year': 1952,
    ...
    charlie_chaplin = Actor(
        {
            'name': 'Charlie Chaplin',
            'favorite_food': [oysters, spaghetti],
            'gross_income_m': 0.371,
            'appearances': 473,
            'is_funny': True,
            'birth_year': 1889,
