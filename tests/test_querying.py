...
def test_operators(backend):
    ...
    for op, results in (
        ('$gt', [david_hasselhoff, charlie_chaplin, marlon_brando]),
        ...
        query = {
            '$and': [
                {'$or': [{'gross_income_m': {op: 1.0}}, {'birth_year': {'$lt': 1900}}]},
                {'$or': [{'is_funny': True}, {'name': 'Marlon Brando'}]},
            ]
        }
    ...
    assert len(
        backend.filter(
            Actor,
            {
                'name': {
                    '$not': {
                        '$in': ['David Hasselhoff', 'Marlon Brando', 'Charlie Chaplin']
                    }
                }
            },
        )
    ...
    assert len(
        backend.filter(
            Actor, {'name': {'$in': ['Marlon Brando', 'Leonardo di Caprio']}}
        )
    ) == 2
