...
def test_circular_reference(backend):
    ...
    francis_coppola = Director(
        {"name": "Francis Coppola", "best_movie": apocalypse_now}
    )
...
def test_basics(backend):
    ...
    assert (
        backend.get(Director, {"movies": {"$all": [the_godfather]}}) == francis_coppola
    )
    assert (
        backend.get(
            Director, {"movies.title": {"$in": ["Apocalypse Now", "The Godfather"]}}
        )
        == francis_coppola
    )
    ...
    result = backend.filter(
        ...
        {
            "actors": {
                "$all": [stanley_kubrick.favorite_actor, francis_coppola.favorite_actor]
            }
        },
    )
